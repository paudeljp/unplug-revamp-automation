...
export function compareTableDataObj(xpathObj, dataPath, environment) {
    grabJsonDataObj(xpathObj, dataPath, environment).then(response => {
        tableP.grabTableData(response.tableXpath, response.tableDatas, response.tableColumns, response.pagination).then(res => {
            expect(JSON.stringify(res.data?.sort())).to.deep.eq(JSON.stringify(response.tableDatas?.sort()))
        })
    })
}
    ...
    grabJsonDataObj(xpathObj, dataPath, environment).then(response => {
        tableP.grabTableData(response.tableXpath, response.tableDatas, response.tableColumns, response.pagination).then(res => {
            if(string == "tbodyCheckbox"){
                buttonP.buttonWithoutContains(response.tableXpath + '> tr:nth-child(' + res.count + ') > td '+ response.allData[string])
            }else{
                buttonP.buttonWithContains(response.tableXpath + '> tr:nth-child(' + res.count + ') > td', string)
            }
        })
    })
}